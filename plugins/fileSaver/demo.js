...
html_options_form.addEventListener("submit", function(event) {
	event.preventDefault();
	if(document.querySelector('#editContent').style.display == 'none'){
		document.querySelector('#downloadTips').style.display = 'block';
		return false;
	}else {
		var
			BB = get_blob()
			...
			, fileType = document.querySelector('#fileTypeShow').innerText || '.html'
			;
		if(fileType == '.txt'){
            saveAs(
                new BB(
                ...
                , (html_filename.value || html_filename.placeholder) + fileType
            );
		}else {
			saveAs(
				new BB(
